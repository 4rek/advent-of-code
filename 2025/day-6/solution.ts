export function partOne(data: string[]): number {
  ...
  const operators = data[data.length - 1].trim().split(" ").filter(n => n.trim())
  const values = new Map<string, number[]>();

  for (let i = 0; i < lines - 1; i++) {
    const line = data[i].trim().split(" ").filter(n => n.trim()).map(Number);
    for (let idx in line) {
...
export function partTwo(data: string[]): number {
  ...
  for (let j = 0; j < values.size; j++) {
    ...
    for (let l = 0; l < newNumbersMap.size; l++) {
      const n = newNumbersMap.get(l.toString())?.filter(n => n !== "0").join("")
